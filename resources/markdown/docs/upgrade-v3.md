...
- Fix All queues gained an apply-all action that writes every ready suggestion at once using each item's reviewed or edited code, with per-item failures reported and rate limiting retried once; previously each ready suggestion had to be accepted individually
